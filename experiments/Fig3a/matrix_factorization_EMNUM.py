...
NITER  = 300001 # number of iterations - around 70k is the right number for mininbatch=10
...
M = args.get("M", 10000.0 if UTIL.lower()=="bigm" else 1.0)
...
UTILITY_TERM_SCALE = args.get("UTILITY_TERM_SCALE",
                              (1.0/M if "linear" in UTILITY_TERM else M))  #for linearized we use U~=-risk 
...
FIND_H = args.get("FIND_H", "num-util")
...
for i in range(NITER):
    ...
    if (len(report)==0 or aux_time.AccumulatingTimer.get_elapsed("LCVI-EM")-report[-1][0]>=200): #report results
        ...
        if aux_time.AccumulatingTimer.get_elapsed("LCVI-EM")>24000: break
