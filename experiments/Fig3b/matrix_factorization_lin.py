...
UTILITY_TERM = args.get("UTILITY_TERM", "linearized") # vi/naive/linearized/jensen
# the utility-dependent term is multiplied by this value: 
#  should be 1/M for linearized and M for the others
UTILITY_TERM_SCALE = args.get("UTILITY_TERM_SCALE", 1.0/args["M"])  #for linearized we use U~=-risk 
...
for i in range(NITER):
    ...
    if (i>0) and (i%EVAL_NITER==0 or i==NITER-1): #report results
        ...
        OUT = "MF_LIN_%s%s_%i_%s_%s" % (UTILITY_TERM, UTILITY_TERM_SCALE, SEED, LOSSSTR, UTIL)
