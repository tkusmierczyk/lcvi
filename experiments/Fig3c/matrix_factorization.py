...
GAIN_OPTIMAL_H_NUMERICALLY = True #True!
...
for i in range(NITER):
    ...
    if (i>0) and (i%EVAL_NITER==0 or i==NITER-1): #report results
        ...
        r = (time.time()-start, utility_term.__name__, SEED, i, epoch_no,
                 elbo.item(), 
                 training_rmse.item(), 
...
pd.DataFrame(report).to_csv("MF_LCVI_%s%s_%i_%s_%s.csv" % (UTILITY_TERM, UTILITY_TERM_SCALE, SEED, LOSS, UTIL), header=False, index=False)
